...
@class NSString;
@class LIAudienceNetworkConfiguration;
...
@interface LIAudienceNetwork : NSObject
/// The Bidder Token to be passed to Bid Requests sent to Linkedin via the OpenRTB spec.
/// This token is available if it was previously cached or upon successful completion of the <code>start</code> method.
/// It will be internally cached and immediately available in subsequent calls.
/// warning:
/// If accessed before <code>start</code> this value will return an empty string.
/// warning:
/// Do not cache this value, instead, access it whenever needed.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull bidderToken;)
+ (NSString * _Nonnull)bidderToken SWIFT_WARN_UNUSED_RESULT;
/// The release version of the SDK.
/// Format follows Semantic Versioning (ex: “0.0.1”)
...
/// \param completion The <code>completion</code> callback when the SDK finishes the start process, may be called on a background queue.
///
+ (void)startWithConfiguration:(LIAudienceNetworkConfiguration * _Nonnull)configuration completion:(void (^ _Nullable)(NSError * _Nullable))completion;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

enum LILogVerbosityConfiguration : NSInteger;
@class NSNumber;
...
@interface LIAudienceNetworkConfiguration : NSObject
/// Level of verbosity desired for logs printed to the console by the SDK
@property (nonatomic) enum LILogVerbosityConfiguration logVerbosity;
/// Determines whether non-personally identifiable data collection is allowed for diagnostic purposes. Default is true.
...
typedef SWIFT_ENUM_NAMED(NSInteger, LILogVerbosityConfiguration, "LogVerbosityConfiguration", open) {
/// Do not log any information to the console
  LILogVerbosityConfigurationNone = 0,
/// Logs errors that occured in the SDK to the console
  LILogVerbosityConfigurationErrors = 1,
/// Logs all information that occured in the SDK to the console
  LILogVerbosityConfigurationAll = 2,
};
